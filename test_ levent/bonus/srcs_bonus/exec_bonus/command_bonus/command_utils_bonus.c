 ...
 */
void	process_child_status(int status, t_minishell *minishell)
{
	if (WIFEXITED(status))
		minishell->exit_code = WEXITSTATUS(status);
	else if (WIFSIGNALED(status))
		minishell->exit_code = SIGNAL_EXIT_BASE + WTERMSIG(status);
	else
		minishell->exit_code = GENERAL_ERROR;
}
...
void	wait_for_process(pid_t pid, t_minishell *minishell)
{
	...
	process_child_status(status, minishell);
}
