...
// Test function for logical operators (AND, OR)
void test_logical_operators(t_minishell *minishell)
{
    printf("\n=== LOGICAL OPERATORS TEST ===\n");
    
    // Create output files for results
    char filepath_and[1024];
    char filepath_or[1024];
    
    if (getcwd(filepath_and, sizeof(filepath_and)) == NULL) {
        printf("Error getting current directory\n");
        return;
    }
    strcpy(filepath_or, filepath_and);
    strcat(filepath_and, "/and_results.txt");
    strcat(filepath_or, "/or_results.txt");
    
    // Test 1: Basic AND operator (success && success)
    printf("\n--- Test 1: AND operator (true && true) ---\n");
    
    // First command (echo "First command")
    t_command_tree *echo1_cmd = create_simple_command(minishell, "echo", 
        (char *[]){"First command succeeded", NULL});
    
    // Second command (echo "Second command")
    t_outfile *output1 = create_outfile(minishell, filepath_and, OUT_TRUNC);
    t_command_tree *echo2_cmd = create_command_with_redirections(minishell, "echo", 
        (char *[]){"AND operator worked!", NULL}, NULL, output1);
    
    // Create AND node
    t_command_tree *and_node = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    and_node->type = AND_OPERATOR;
    and_node->data = NULL;
    and_node->left = echo1_cmd;
    and_node->right = echo2_cmd;
    
    run_test(minishell, and_node, "AND operator (echo success && echo success)");
    
    // Test 2: Basic AND operator (failure && success - should not execute second command)
    printf("\n--- Test 2: AND operator (false && true) ---\n");
    
    // First command (non-existent command that will fail)
    t_command_tree *nonexist_cmd = create_simple_command(minishell, "grep", 
        (char *[]){"nonexistentpattern", "/etc/passwd", NULL});
    
    // Second command (should not run)
    t_outfile *output2 = create_outfile(minishell, filepath_and, OUT_APPEND);
    t_command_tree *echo3_cmd = create_command_with_redirections(minishell, "echo", 
        (char *[]){"This should NOT appear in the output file", NULL}, NULL, output2);
    
    // Create AND node
    t_command_tree *and_node2 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    and_node2->type = AND_OPERATOR;
    and_node2->data = NULL;
    and_node2->left = nonexist_cmd;
    and_node2->right = echo3_cmd;
    
    run_test(minishell, and_node2, "AND operator (grep failure && echo success)");
    
    // Test 3: Basic OR operator (failure || success - should execute second command)
    printf("\n--- Test 3: OR operator (false || true) ---\n");
    
    // First command (will fail)
    t_command_tree *grep_fail_cmd = create_simple_command(minishell, "grep", 
        (char *[]){"nonexistentpattern", "/etc/passwd", NULL});
    
    // Second command (should run)
    t_outfile *output3 = create_outfile(minishell, filepath_or, OUT_TRUNC);
    t_command_tree *echo4_cmd = create_command_with_redirections(minishell, "echo", 
        (char *[]){"OR operator worked when first command failed!", NULL}, NULL, output3);
    
    // Create OR node
    t_command_tree *or_node = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or_node->type = OR_OPERATOR;
    or_node->data = NULL;
    or_node->left = grep_fail_cmd;
    or_node->right = echo4_cmd;
    
    run_test(minishell, or_node, "OR operator (grep failure || echo success)");
    
    // Test 4: OR operator (success || success - should not execute second command)
    printf("\n--- Test 4: OR operator (true || true) ---\n");
    
    // First command (will succeed)
    t_command_tree *echo5_cmd = create_simple_command(minishell, "echo", 
        (char *[]){"First command succeeded in OR", NULL});
    
    // Second command (should not run)
    t_outfile *output4 = create_outfile(minishell, filepath_or, OUT_APPEND);
    t_command_tree *echo6_cmd = create_command_with_redirections(minishell, "echo", 
        (char *[]){"This should NOT appear in the OR output file", NULL}, NULL, output4);
    
    // Create OR node
    t_command_tree *or_node2 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or_node2->type = OR_OPERATOR;
    or_node2->data = NULL;
    or_node2->left = echo5_cmd;
    or_node2->right = echo6_cmd;
    
    run_test(minishell, or_node2, "OR operator (echo success || echo success)");
    
    // Test 5: Complex combination (A && (B || C))
    printf("\n--- Test 5: Complex combination (A && (B || C)) ---\n");
    
    // Command A - will succeed
    t_command_tree *cmdA = create_simple_command(minishell, "echo", 
        (char *[]){"Command A succeeded", NULL});
    
    // Command B - will fail
    t_command_tree *cmdB = create_simple_command(minishell, "grep", 
        (char *[]){"nonexistentpattern", "/etc/passwd", NULL});
    
    // Command C - will succeed and write to file
    t_outfile *output5 = create_outfile(minishell, filepath_and, OUT_APPEND);
    t_command_tree *cmdC = create_command_with_redirections(minishell, "echo", 
        (char *[]){"Complex expression (A && (B || C)) worked!", NULL}, NULL, output5);
    
    // Create B || C
    t_command_tree *B_or_C = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    B_or_C->type = OR_OPERATOR;
    B_or_C->data = NULL;
    B_or_C->left = cmdB;
    B_or_C->right = cmdC;
    
    // Create A && (B || C)
    t_command_tree *complex_node = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    complex_node->type = AND_OPERATOR;
    complex_node->data = NULL;
    complex_node->left = cmdA;
    complex_node->right = B_or_C;
    
    run_test(minishell, complex_node, "Complex logical expression (A && (B || C))");
    
    // Display the output files
    printf("\nAND operator test results:\n");
    int fd_and = open(filepath_and, O_RDONLY);
    if (fd_and >= 0) {
        char buffer[1024];
        ssize_t bytes_read = read(fd_and, buffer, sizeof(buffer) - 1);
        if (bytes_read > 0) {
            buffer[bytes_read] = '\0';
            printf("%s\n", buffer);
        }
    ...
    printf("\nOR operator test results:\n");
    int fd_or = open(filepath_or, O_RDONLY);
    if (fd_or >= 0) {
        char buffer[1024];
        ssize_t bytes_read = read(fd_or, buffer, sizeof(buffer) - 1);
        if (bytes_read > 0) {
            buffer[bytes_read] = '\0';
            printf("%s\n", buffer);
        }
    ...
    printf("\n=== LOGICAL OPERATORS TEST COMPLETED ===\n");
}

// Test function for logical operators exit code compliance
void test_logical_operators_exit_codes(t_minishell *minishell)
{
    printf("\n=== LOGICAL OPERATORS EXIT CODE TEST ===\n");
    
    // Create commands with specific exit codes
    // true: always returns 0

    
    // Custom exit codes using bash
    char *exit2_args[] = {"-c", "exit 2", NULL};

    
    char *exit3_args[] = {"-c", "exit 3", NULL};

    // Test 1: (true && exit2) - should be 2
    t_command_tree *and1 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    and1->type = AND_OPERATOR;
    and1->data = NULL;
    and1->left = create_simple_command(minishell, "true", NULL); // Create fresh copy
    and1->right = create_simple_command(minishell, "bash", exit2_args);
    run_test(minishell, and1, "Test 1: (true && exit2) [Expected: 2]");
    
    // Test 2: (false && exit2) - should be 1
    t_command_tree *and2 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    and2->type = AND_OPERATOR;
    and2->data = NULL;
    and2->left = create_simple_command(minishell, "false", NULL);
    and2->right = create_simple_command(minishell, "bash", exit2_args);
    run_test(minishell, and2, "Test 2: (false && exit2) [Expected: 1]");
    
    // Test 3: (true || exit2) - should be 0
    t_command_tree *or1 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or1->type = OR_OPERATOR;
    or1->data = NULL;
    or1->left = create_simple_command(minishell, "true", NULL);
    or1->right = create_simple_command(minishell, "bash", exit2_args);
    run_test(minishell, or1, "Test 3: (true || exit2) [Expected: 0]");
    
    // Test 4: (false || true) - should be 0
    t_command_tree *or2 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or2->type = OR_OPERATOR;
    or2->data = NULL;
    or2->left = create_simple_command(minishell, "false", NULL);
    or2->right = create_simple_command(minishell, "true", NULL);
    run_test(minishell, or2, "Test 4: (false || true) [Expected: 0]");
    
    // Test 5: (false || exit3) - should be 3
    t_command_tree *or3 = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or3->type = OR_OPERATOR;
    or3->data = NULL;
    or3->left = create_simple_command(minishell, "false", NULL);
    or3->right = create_simple_command(minishell, "bash", exit3_args);
    run_test(minishell, or3, "Test 5: (false || exit3) [Expected: 3]");
    
    // Test 6: Complex - (true && (false || exit2)) - should be 2
    t_command_tree *or_inner = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    or_inner->type = OR_OPERATOR;
    or_inner->data = NULL;
    or_inner->left = create_simple_command(minishell, "false", NULL);
    or_inner->right = create_simple_command(minishell, "bash", exit2_args);
    
    t_command_tree *complex = gc_malloc(minishell->gc[GC_COMMAND], sizeof(t_command_tree));
    complex->type = AND_OPERATOR;
    complex->data = NULL;
    complex->left = create_simple_command(minishell, "true", NULL);
    complex->right = or_inner;
    run_test(minishell, complex, "Test 6: (true && (false || exit2)) [Expected: 2]");
    
    printf("\n=== LOGICAL OPERATORS EXIT CODE TEST COMPLETED ===\n");
}
...
int main(int argc, char **argv, char **envp)
{
    ...
    // test_logical_operators(&minishell);
    test_logical_operators_exit_codes(&minishell);
