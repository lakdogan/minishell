...
int main(int argc, char **argv, char **envp) {
    ...
    //======================= LOGICAL OPERATORS ======================
    print_category_header("LOGICAL OPERATORS");
    
    // AND operator
    t_command_tree *true_cmd = create_cmd_node(
        "/usr/bin/true", 
        create_args(1, "true"), 
        NULL, 
        NULL
    ...
    t_outfile *outfile6 = create_outfile("and_test.txt", OUT_TRUNC);
    t_command_tree *echo_and = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "AND operator works"), 
        NULL, 
        outfile6
    );
    
    t_command_tree *and_node = create_and_node(true_cmd, echo_and);
    run_test("Logical AND (&&)", and_node, &sh, 0, "and_test.txt", "AND operator works\n");
    
    // OR operator
    t_command_tree *false_cmd = create_cmd_node(
        "/usr/bin/false", 
        create_args(1, "false"), 
        NULL, 
        NULL
    ...
    t_outfile *outfile7 = create_outfile("or_test.txt", OUT_TRUNC);
    t_command_tree *echo_or = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "OR operator works"), 
        NULL, 
        outfile7
    );
    
    t_command_tree *or_node = create_or_node(false_cmd, echo_or);
    run_test("Logical OR (||)", or_node, &sh, 0, "or_test.txt", "OR operator works\n");
    
    // Complex nested logic
    t_command_tree *echo_begin = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "Begin"), 
        NULL, 
        NULL
    );
    
    t_command_tree *ls_complex = create_cmd_node(
        "/bin/ls", 
        create_args(2, "ls", "-la"), 
        NULL, 
        NULL
    ...
    t_command_tree *grep_complex = create_cmd_node(
        "/usr/bin/grep", 
        create_args(2, "grep", ".c"), 
        NULL, 
        NULL
    );
    
    t_command_tree *wc_complex = create_cmd_node(
        "/usr/bin/wc", 
        create_args(2, "wc", "-l"), 
        NULL, 
        NULL
    ...
    t_command_tree *echo_fail = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "Failed"), 
        NULL, 
        NULL
    );
    
    t_command_tree *pipe_complex1 = create_pipe_node(ls_complex, grep_complex);
    t_command_tree *pipe_complex2 = create_pipe_node(pipe_complex1, wc_complex);
    t_command_tree *and_complex = create_and_node(echo_begin, pipe_complex2);
    t_command_tree *or_complex = create_or_node(and_complex, echo_fail);
    
    run_test("Complex nested logic", or_complex, &sh, 0, NULL, NULL);
    ...
    t_command_tree *echo_success = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "This should NOT be printed"), 
        NULL, 
        NULL
    );
    
    t_command_tree *echo_failure = create_cmd_node(
        "/bin/echo", 
        create_args(2, "echo", "Exit code properly propagated"), 
        NULL, 
        NULL
    );
    
    t_command_tree *and_exit = create_and_node(exit42_cmd, echo_success);
    t_command_tree *or_exit = create_or_node(and_exit, echo_failure);
    
    run_test("Exit code propagation", or_exit, &sh, 0, NULL, NULL);
