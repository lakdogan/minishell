...
t_token				init_token(const char *cmd, int *i,
						const int t_count, t_minishell *shell);
...
int					handle_redir(t_exec	*exec, t_tokens *tokens,
						int *pos, t_gc *gc);
...
t_arg_lst			*get_args(t_exec *exec, t_tokens *tokens,
						int *pos, t_gc *gc);
