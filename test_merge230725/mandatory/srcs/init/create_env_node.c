...
static t_env	*allocate_env_node(t_minishell *shell)
{
    ...
    if (!env)
        exit_with_error(shell, "minishell: ", "env struct", "Memory allocation failed", EXIT_FAILURE);
...
if (!env->value)
	exit_with_error(shell, "minishell: ", "env value", "Memory allocation failed", EXIT_FAILURE);
...
static void	set_env_content(t_minishell *shell, t_env *env, char *env_str)
{
    ...
    if (!env->content)
        exit_with_error(shell, "minishell: ", "env content", "Memory allocation failed", EXIT_FAILURE);
}
